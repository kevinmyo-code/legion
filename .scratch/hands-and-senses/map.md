...
| 13 | **Proactivity is a master switch PLUS per-category control** (Kevin, 2026-08-16). One toggle is the primary control; categories inherit it and are tunable after. | Closes half of [The proactive switch](issues/21-proactive-mode.md); the category list, tri-state-or-not, and whether safety is exempt from the master remain open there. No other ticket invents a separate opt-in. |
