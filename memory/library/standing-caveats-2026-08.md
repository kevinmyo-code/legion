...
# Standing caveats, blockers and device traps (as of 2026-08-28)

**LIVE.** Lifted verbatim from `memory/MEMORY.md` on 2026-08-28 when the dashboard was cut back to
its 80-line cap. These are the things a session needs to know before it trusts a green suite: what
is blocking, what has never actually been exercised, what was in flight, and the ADB/device traps
that have cost real time more than once.

Nothing here was edited in the move. Where an item says "still owed", verify it against the code
before acting - some of these entries predate the backend-ERP arc.
