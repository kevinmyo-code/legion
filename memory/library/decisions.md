...
(`currentTransaction_4146.csv`, read then left alone, never copied into the repo) showed it is the
...
`"4146"` and the card PDF's is `"4111111111114146"`, and every ledger mechanism keys on equality, so
...
checking total -3,115,676 cents, and only 10 `PAYMENT FROM CHK` legs on the card were ever imported.
So ~24,000 dollars of his own money moving between his own accounts was counted as spending, on top
...
catches three different things: card payments (75 rows, -3,115,676c), own SAV/CHK transfers (19 rows,
+973,103c), and **40 `Zelle payment to <person>` rows worth -666,980c, which are real money leaving,
not transfers at all**. The blunt rule would have hidden 6,670 dollars of genuine payments - the
