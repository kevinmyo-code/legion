...
- **Commit identity.** This repo's git config used a work email address. Kevin
...
`resolveDedup`, where loosening the key would let a checking account ending 7823 absorb card rows.
...
checking summed to a five-figure sum of his own card payments (in cents), and only 10 `PAYMENT FROM
CHK` legs on the card were ever imported. So tens of thousands of dollars of his own money moving
between his own accounts was counted as spending, on top of the purchases it was paying for.
...
catches three different things: card payments (75 rows, a five-figure sum in cents), own SAV/CHK
transfers (19 rows, several thousand dollars in cents), and **40 `Zelle payment to <person>` rows
worth several thousand dollars across 40 person-to-person rows, which are real money leaving,
not transfers at all**. The blunt rule would have hidden thousands of dollars of genuine payments -
the opposite of the bug being fixed, and precisely what the 2026-08-07 note anticipated. **Kevin was
...
2. **VIN was decoded but never written back.** `vehicle_specs` holds a fully decoded 1FAKEVIN000000001 (6cyl 4.0L in-line, FCA, Toledo), decoded 2026-07-26, **on `vehicleId = 12:34:56:11:22:33` - the SAME row as the blank vehicle**. The two tables have disagreed for three weeks with nothing noticing. The `check_recalls` gate passes on `confirmed=1`, and then `VinDecoder.fetchRecalls` guards at its own entry (`if (year <= 0 || make.isBlank() || model.isBlank()) return@withContext emptyList()`, `VinDecoder.kt:98-99`) - so **no request is made at all** and an empty list comes back, which is indistinguishable from "no open recalls" on a car the system never asked about. **Corrected 2026-08-15**: the original wording here claimed it queried NHTSA with empty parameters. Wrong in mechanism, unchanged in consequence, and asserted before `VinDecoder` had been read.
...
**Event:** 2026-08-17, Kevin spoke a request to log several meals (he reported it mid-session; the exact clock time was never established, and the last meal row before the fix predates the day's dispatcher commit by well under an hour). UI sat on "Listening..." ~5 minutes, nothing logged. Reported same session. [on-device, user-reported]

**Root chain:** Commit b1868d8 (same morning) put five domains behind ask_<domain> dispatchers to cut the re-billed Live setup block from 79 declarations to 45. That moved log_meal out of a flat live tool and behind ask_body -> SubAgent.investigate (maxModelCalls 4), and each round's log_meal makes its OWN nested blocking SubAgent.ask for the macro estimate. **Blocking HTTP nested inside blocking HTTP, with every governing timeout inert per L29.**

**Proof the dispatched path had never worked:** meal_logs held 7 rows, and the newest predates b1868d8 (the day's dispatcher commit) by 43 minutes. Not one row written through ask_body. [on-device, DB pulled off the phone and queried with WAL]
...
**Verification after fix:** same request re-spoken on a hash-verified install (75a78f6c...), meal_logs rows 8 and 9 written in the same second (two log_meal calls in one investigate round). trustTier REPORTED, macro estimates recorded. [on-device]
...
| Claim | Tag |
|---|---|
| ask_body dispatcher never written through before b1868d8 | **`on-device`** - DB pulled and queried; newest pre-fix row predates the commit by 43 minutes, next row written well after fix |
...
## 2026-08-25 - Backend-ERP grilling: The xlsx mirror and the local cache in a backend world ([[04-mirror-and-cache-fate]], RESOLVED)

Four rulings on audit surfaces, hand-edit workflows, cache shape, and recovery. One critical finding: the import half of the mirror never round-tripped on a device, yet the research nominated "the mirror" wholesale as the recovery story. Audit surface, editing, and cache were each evaluated under a backend model; recovery revealed a pre-existing gap disguised as a decided answer.

1. **The xlsx mirror is RETIRED ENTIRELY (Kevin, 2026-08-25).** Not export-only: removed entirely. Supabase dashboard's table and SQL editors become the audit surface. Two facts justify it beyond audit preference: (a) the import half has never round-tripped on a device - export WAS verified on the A25 on 2026-08-23 (SAF create, rwt rewrite, SHA-256 read-back, no quarantine); import was JVM-unit-tested only (`MirrorSyncMergeTest`, 12 tests; `MirrorCodecTest`, 7; no on-device hand-edit workflow ever run). (b) The mirror is entirely generic-shape dependent (`MirrorCodec` takes Aspect/RecordType/FieldDef/EngineRecord as vocabulary), so ticket 01 ruling 7 was going to collapse it regardless. Retires roughly 2,200 lines: `engine/mirror/` (1,423) + `ui/mirror/MirrorSyncActivity.kt` (186) + `MirrorSyncMergeTest` (519) + `MirrorCodecTest` (232). Also retires a known dishonesty: "protected" columns were a header string, never a cell lock, because fastexcel 0.19.0 has no per-cell lock API.

2. **Hand-edit-and-reimport does NOT survive (Kevin, 2026-08-25).** The import path was the load-bearing risk. A blank-guid row with content created a record with provenance USER; a row carrying a foreign guid was created preserving that guid - a spreadsheet could mint records. The gate's read-only protection for DETERMINISTIC/LLM_RECONCILED rows was real and tested, but it protected only reconciled rows, not the act of creating new ones. Nothing writes past the reconciliation gate through a spreadsheet any more.

3. **The local cache is a FULL REPLICA of the household's records (Kevin, 2026-08-25).** Measured: 569 active engine records (verified on device by pulling the real DB), roughly 285 KB estimated footprint, worst case 1.1 MB. Against the free tier's 500 MB ceiling that is three orders of magnitude of headroom, so windowing would add cache-invalidation complexity to defend against a number that rounds to zero. Photos are excluded by construction - ticket 03 already sends them to Supabase Storage.

4. **Recovery moves to sync/DatabaseSnapshot.kt, SCHEDULED, with a restore actually exercised (Kevin, 2026-08-25).** The research said "the mirror carries real weight as the recovery story" because the free tier has zero backup retention. That claim was already false in practice - it rested on an import path that had never round-tripped on a device. DatabaseSnapshot is already built: whole-DB gzip plus metadata to Drive appDataFolder, 3 generations, pruned only after new upload confirmed. The restore refuses a newer schema than running build, sniffs SQLite magic bytes, takes a local safety snapshot, and rolls back on failure. **Two things are owed and both are binding.** First, it is manual-only - sole callers are three buttons in DriveSyncScreen; it needs a scheduler. Second, a restore has never been exercised on a device (NOT ESTABLISHED). An untested restore is not a backup, it is a hope - exactly the mistake ruling 1 just corrected in the mirror. Do not retire the mirror until the scheduled snapshot and a real restore are both done, or there is a window with no recovery path at all.

**Consequences for ticket 05.** The five retirements it now owns, across the whole map, are: the generic record engine (ticket 01 ruling 7), the Notes `Item` type merging into the Dates `Event` type (ticket 01 ruling 4), Google Calendar (ticket 01 ruling 5), the xlsx mirror (this ticket), and the deterministic statement parsers (ticket 03 ruling 3). **`DatabaseSnapshot` is NOT among them - ruling 4 KEEPS it** and adds a scheduler plus a device-exercised restore; deleting it would remove the only recovery path. Separately, three sync mechanisms are live simultaneously today and ticket 05 must sequence all three: MirrorSync (retired here), SyncEngine/SyncMerge (row-level merge over roughly 19 legacy tables, still auto-triggered every 5 minutes and **still unruled by any ticket**), and DatabaseSnapshot (kept, currently manual). Aspect-engine ticket 20 declared SyncEngine retired for record data but retired zero legacy tables, which is why it is still running. Also a read-path state vocabulary gap: ledger, pantry, fleet have no loading state (empty and not-yet-loaded are the same pixels), no error state, and no stale visibility despite ruling 9 of ticket 01 (cache-first reads). WidgetDataSource already distinguishes empty/not-configured/error in words; extend it rather than invent a second vocabulary.
