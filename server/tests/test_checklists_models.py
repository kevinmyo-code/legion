...
def test_updated_at_touch_trigger_overrides_whatever_the_caller_supplies():
    ...
    """
    checklist = Checklist.objects.create(name="bio")
...
def test_created_at_and_updated_at_are_stamped_by_the_database_not_left_null():
    checklist = Checklist.objects.create(name="bio")
    assert checklist.created_at is not None
    assert checklist.updated_at is not None


def test_item_day_unique_constraint_holds_even_across_a_soft_delete():
    checklist = Checklist.objects.create(name="bio")
...
def test_revive_a_soft_deleted_tick_via_update_not_a_second_insert():
    checklist = Checklist.objects.create(name="bio")
    item = ChecklistItem.objects.create(checklist=checklist, text="squats")
...
def test_measured_tick_guard_trigger_refuses_a_valueless_tick_bypassing_the_serializer():
    ...
    checklist = Checklist.objects.create(name="fitness")
...
def test_measured_tick_guard_trigger_allows_a_valued_tick():
    checklist = Checklist.objects.create(name="fitness")
    measured_item = ChecklistItem.objects.create(
        checklist=checklist, text="walk 10k steps", measure_unit="steps"
    )
...
def test_measured_tick_guard_trigger_leaves_a_binary_item_untouched():
    checklist = Checklist.objects.create(name="chores")
