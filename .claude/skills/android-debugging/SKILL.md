...
For deeper Compose performance analysis (stability, recomposition skipping), see the vendored `compose-recomposition-performance` and `compose-stability-diagnostics` skills. *(Upstream pointed at `android-skills:compose` → `references/performance.md`, which is not vendored here.)*
