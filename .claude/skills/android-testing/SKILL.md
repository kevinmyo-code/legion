...
Android-specific testing. This reference focuses on the Compose-test traps that are easy to get wrong, not the basics of the three tiers, fakes over mocks, `runTest`, or Given-When-Then naming.

*(Vendoring note: upstream's "Test-first (the foundation)" section was stripped here — TDD discipline is the `mattpocock-skills:tdd` plugin's ground. For bootstrapping the test stack from scratch, see the locally vendored `testing-setup` skill.)*
...
| Focus navigation, keyboard, TV/D-pad | Compose test with `performKeyInput` + `assertIsFocused()` (see the vendored `compose-focus-navigation` skill) |
