...
class MidnightApplication : Application() {
    ...
    override fun onCreate() {
        ...
        if (!isRunningUnderRobolectric()) {
            ...
            // Body Realtime (body-supabase ticket) - same shape and reasoning as EventsRealtime
            // just above; see BodyRealtime's own class doc for the full lifecycle contract.
            ...
            // Ledger config Realtime (ledger-config-supabase ticket) - same shape and reasoning as
            // MemoryRealtime just above; see LedgerConfigRealtime's own class doc for the full
            // lifecycle contract.
            runCatching { com.kevin.legion.backend.LedgerConfigRealtime.bind(this@MidnightApplication) }
                .onFailure { MidnightEvents.appStartWorkFailed("bind_ledger_config_realtime", it) }

            // Last-aspects Realtime (live-sync's last aspect slice) - same shape and reasoning as
            // LedgerConfigRealtime just above; see LastAspectsRealtime's own class doc for the
