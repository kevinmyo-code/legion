...
class DefaultArrangementSeeder(
    ...
    suspend fun seedHomeIfEmpty(deviceId: String, now: Long = System.currentTimeMillis()) {
        if (!widgetStore.isDeviceEmpty(deviceId)) return

        // AGENDA first, full-width and tall - the single most information-dense default, and the
        // one TODAY's own "coming up" pane already leads with. Empty today (no `dueAt` records
        // exist anywhere on a fresh engine) - the widget's own empty-state copy says so in words.
        ...
        // NEXT-DUE, full-width row directly under the agenda - a maintenance item, a bill, whatever
        // the engine's `dueAt` promoted column surfaces first once a real aspect starts writing it.
        widgetStore.addWidget(
            deviceId = deviceId,
            aspectId = null,
            recordTypeId = null,
        ...
        // half-tile row is exactly this shape (EqualHeightRow's two-up grammar), so the seed mirrors
        // it rather than inventing a new density. A tile with no `fieldId`/`aggregateOp` configured
        // (`config = "{}"`) is a DELIBERATE seed state: it reads "not configured yet" in words
        // (`ui/widgets/EngineWidgets.kt`'s `StatTileWidget`) rather than a fabricated number.
