...
interface SleepLogDao {
    ...
    @Query("SELECT * FROM sleep_logs ORDER BY sleepDate DESC, loggedAt DESC LIMIT :limit")
     ...
     */
    @Query("SELECT * FROM sleep_logs WHERE sleepDate >= :fromMs AND sleepDate < :toMs")
    suspend fun forWindow(fromMs: Long, toMs: Long): List<SleepLog>

    @Query("SELECT * FROM sleep_logs ORDER BY loggedAt DESC, id DESC LIMIT 1")
