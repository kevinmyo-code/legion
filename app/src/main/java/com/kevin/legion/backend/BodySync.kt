...
object BodySync {
    ...
    fun maybeAutoPull(context: Context) {
        ...
        // rather than a raw SupabaseClientProvider read, so a device whose `body` row is flipped
        // to Django is not turned away for having no Supabase project - the same substitution
        // EventsSync.maybeAutoPull's own doc comment records for `events`. The throttle slot is
        // still reserved before the launch, unchanged.
        ...
        autoPullScope.launch {
            try {
                ...
                val backend = backends.bodyBackend() ?: return@launch
