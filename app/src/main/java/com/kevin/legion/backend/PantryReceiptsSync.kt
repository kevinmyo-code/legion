...
object PantryReceiptsSync {
    ...
    fun maybeAutoPull(context: Context) {
        ...
        // rather than a raw SupabaseClientProvider read, so a device whose `pantry` row is flipped
        ...
        if (!backends.isConfiguredFor(EngineBackends.ASPECT_PANTRY)) return
        val onDjango = EngineTransport(app).transportFor(EngineBackends.ASPECT_PANTRY) == Transport.DJANGO
        lastAutoPullAt = now
        autoPullScope.launch {
            try {
                ...
                val backend = backends.pantryBackend() ?: return@launch
