...
object FleetRealtime {
    ...
    //
    // The transport switch (django-engine Phase 5), same guard [LedgerConfigRealtime.subscribe]
    ...
    // project it no longer reads. Folded into ONE guard with the pre-existing
    // `channels.isNotEmpty()` check, same shape, to stay inside detekt's two-return ceiling
    // without a suppression.
    private fun subscribe(context: Context) {
        val onDjango =
            EngineTransport(context).transportFor(EngineBackends.ASPECT_FLEET) == Transport.DJANGO
