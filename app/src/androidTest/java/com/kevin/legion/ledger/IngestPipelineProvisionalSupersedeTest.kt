...
class IngestPipelineProvisionalSupersedeTest {
    ...
    private fun provisionalTxn(txnDate: Long, description: String, amountCents: Long, accountId: String = "4146") =
        LedgerTransaction(
    ...
    fun reconciledStatementSupersedesProvisionalRowsInItsWindow() = kotlinx.coroutines.runBlocking {
        ...
        assertEquals(2, txnDao.getForAccountInRange("4146", JUL_1, JUL_10).size)

        val reconciled = commitLikeIngestPipeline(
            txnDao, ingestedDao, "card-pdf-1",
            listOf(
                reconciledTxn(JUL_1, "NORTHWIND OUTFITTERS.COM", -4500L, "5555555555554146"),
                reconciledTxn(JUL_10, "GREENFIELD MARKET #2", -1200L, "5555555555554146"),
            ),
        )

        assertEquals(2, reconciled.provisionalSuperseded)
        assertEquals(2, reconciled.inserted)
        assertTrue("provisional rows must be gone", txnDao.getForAccountInRange("4146", JUL_1, JUL_10).isEmpty())
        assertEquals(2, txnDao.getForAccountInRange("5555555555554146", JUL_1, JUL_10).size)
    }
    ...
    fun aReconciledStatementForADifferentWindowLeavesProvisionalRowsAlone() = kotlinx.coroutines.runBlocking {
        ...
        val reconciled = commitLikeIngestPipeline(
            ...
            listOf(reconciledTxn(JUN_5, "SOME JUNE PURCHASE", -900L, "5555555555554146")),
        )

        assertEquals(0, reconciled.provisionalSuperseded)
        assertEquals(1, txnDao.getForAccountInRange("4146", JUL_1, JUL_1).size)
    }
