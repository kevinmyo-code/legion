...
class BofaCardCsvStatementParserTest {
    ...
    fun `recognizes the header and parses three invented rows into three transactions`() {
        ...
        val transactions = BofaCardCsvStatementParser.parse("currentTransaction_4146.csv", csv.byteInputStream())
    ...
    fun `wrong first line falls through as unrecognized, not a hard failure`() {
        ...
        assertThrows(UnrecognizedLayoutException::class.java) {
            BofaCardCsvStatementParser.parse("currentTransaction_4146.csv", csv.byteInputStream())
        }
    }
    ...
    fun `header with no data rows fails rather than committing an empty import`() {
        assertThrows(StatementParseException::class.java) {
            BofaCardCsvStatementParser.parse("currentTransaction_4146.csv", header.byteInputStream())
        }
    }
    ...
    fun `CRLF input parses identically to LF input`() {
        ...
        val lfResult = BofaCardCsvStatementParser.parse("currentTransaction_4146.csv", lf)
        val crlfResult = BofaCardCsvStatementParser.parse("currentTransaction_4146.csv", crlf)
