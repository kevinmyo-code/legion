...
class LedgerAccountIdentityTest {
    ...
    fun `sameCard matches on last-4 suffix regardless of full account length`() {
        assertTrue(sameCard("4111111111114146", "4146"))
        assertTrue(sameCard("4146", "4111111111114146"))
        assertFalse(sameCard("4111111111114146", "9999"))
    }

    @Test
    fun `a card payment naming the card's own account is an own-account reference`() {
        // Real shape: `Online Banking payment to CRD 4146 Confirmation# 0649409616`.
        assertTrue(
            referencesOwnAccount(
                "Online Banking payment to CRD 4146 Confirmation# 0649409616",
    ...
    fun `a checking-to-card payment naming the checking account is an own-account reference`() {
        // Real shape: `PAYMENT FROM CHK 3119 CONF#m7huqidzh`.
        assertTrue(
            referencesOwnAccount(
                "PAYMENT FROM CHK 3119 CONF#m7huqidzh",
                ownAccountIds = setOf("411111113119"),
            ),
        )
    }
    ...
    fun `a savings transfer naming an account never on file is NOT an own-account reference - conservative by design`() {
        // Real shape: `Online Banking transfer from SAV 1490 Confirmation# 1731436758` - Kevin's
        ...
        assertFalse(
            referencesOwnAccount(
                "Online Banking transfer from SAV 1490 Confirmation# 1731436758",
                ownAccountIds = setOf("411111113119", "4111111111114146"),
            ),
        )
    }
    ...
    fun `a Zelle payment TO a person names no account and is never an own-account reference`() {
        // Real shape: `Zelle payment to  U Naing Win US Conf# b4nb0qacg` - a person's name, not an
        // account digit, so this must never match regardless of what ownAccountIds contains.
        assertFalse(
            referencesOwnAccount(
                "Zelle payment to  U Naing Win US Conf# b4nb0qacg",
                ownAccountIds = setOf("411111113119", "4111111111114146"),
            ),
        )
    }
    ...
    fun `a Zelle payment FROM a person names no account and is never an own-account reference`() {
        // Real shape: `Zelle payment from MYAT YAMIN THU Conf# pwekdcqu8`.
        assertFalse(
            referencesOwnAccount(
                "Zelle payment from MYAT YAMIN THU Conf# pwekdcqu8",
                ownAccountIds = setOf("411111113119", "4111111111114146"),
            ),
        )
    }
    ...
    fun `a merchant address digit run is never mistaken for an account reference`() {
        // Real false-positive risk this repo measured: `CIRCLE K # 41490 CYPRESS TX` contains the
        // digits 41490 (which even ENDS in 1490, the savings account's own last-4) purely by
        // coincidence of a gas station's store number - it must never match because it is never
        // preceded by CRD/CHK/SAV/ACCT.
        assertFalse(
            referencesOwnAccount(
                "CIRCLE K # 41490         CYPRESS      TX",
                ownAccountIds = setOf("411111113119", "4111111111114146", "31490"),
            ),
        )
    }
    ...
    fun `an empty ownAccountIds set never matches anything, even a real CRD reference`() {
        assertFalse(referencesOwnAccount("PAYMENT TO CRD 4146", ownAccountIds = emptySet()))
    }

    @Test
    fun `a description naming no digits at all never matches`() {
        assertFalse(referencesOwnAccount("MOBILE BANKING PAYMENT TO CRD", ownAccountIds = setOf("4111111111114146")))
    }
}
